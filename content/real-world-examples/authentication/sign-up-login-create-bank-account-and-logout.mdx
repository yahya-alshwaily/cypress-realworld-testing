...
We will use <apiLink apiName="wait" displayName="cy.wait()" /> to wait on our intercept which we aliased to `@signup` in the `beforeEach()` hook, to ensure that the user account was created and saved in the database
